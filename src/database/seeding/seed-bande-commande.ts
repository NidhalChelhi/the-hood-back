...
import { BandeCommande, BandeCommandeSchema } from "../../modules/bande-commande/bande-commande.schema";
...
const seedBandeCommande = async () => {
  ...
  const BandeCommandeModel : Model<BandeCommande> = connection.model(BandeCommande.name, BandeCommandeSchema);
  ...
  const users = await UserModel.find({role : UserRole.RestaurantManager}).lean();
...
seedBandeCommande().catch((error) => console.error("Seeding Bande Commande failed: ", error));