...
import { SupplyBatch, SupplyBatchSchema } from '../../modules/supply-batch/supply-batch.schema';
...
const seedBatches = async () => {
  ...
  const BatchModel : Model<SupplyBatch> = connection.model(SupplyBatch.name, SupplyBatchSchema);
  ...
  for(let i = 0; i < products.length * 5; i++){
    ...
    console.log(batchP);
    await ProduitModel.findByIdAndUpdate(new Types.ObjectId(fake_p.toString()), {
...
seedBatches().catch((error) => console.error("Erreur fl supply batch seeding : ", error));