...
export class BandeCommandeService {
  ...
  async findById(id: string) {
    const bandeCommande = await this.bandeCommandeModel.findById(id).exec();
    if (!bandeCommande) {
  ...
  async findOwnById(id: string, userId: string | undefined) {
    if (!userId) {
  ...
  async updateBandeCommande(
    ...
    if (role == UserRole.StockManager || role == UserRole.Admin) {
      const updatedBandeCommande =
        await this.bandeCommandeModel.findByIdAndUpdate(
          id,
